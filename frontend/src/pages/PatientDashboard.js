      ...
      <div className="content-wrapper">
        ...
        <div className="animate-fadeInUp" style={{ background: 'var(--gradient-brand)', borderRadius: '24px', padding: '28px 36px', marginBottom: '28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '20px', boxShadow: '0 16px 50px var(--accent-glow)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display:'flex', gap:'10px', zIndex:1, flexWrap:'wrap' }}>
            <Link to="/predict" style={{ display:'inline-flex', alignItems:'center', gap:'7px', padding:'11px 20px', borderRadius:'13px', background:'rgba(255,255,255,0.18)', border:'1.5px solid rgba(255,255,255,0.3)', color:'white', textDecoration:'none', fontWeight:700, fontSize:'14px', backdropFilter:'blur(6px)', transition:'all 0.2s' }}
              onMouseOver={e=>e.currentTarget.style.background='rgba(255,255,255,0.26)'}
              onMouseOut={e=>e.currentTarget.style.background='rgba(255,255,255,0.18)'}>
              🔬 Run Prediction
            </Link>
            <Link to="/doctors" style={{ display:'inline-flex', alignItems:'center', gap:'7px', padding:'11px 20px', borderRadius:'13px', background:'white', color:'#059669', textDecoration:'none', fontWeight:800, fontSize:'14px', transition:'all 0.2s', boxShadow:'0 4px 14px rgba(0,0,0,0.2)' }}
        ...
        <div className="grid-4" style={{ marginBottom:'24px' }}>
          {[
            ...
            { emoji:'📋', label:'Reports',      val:predictions.length,                                       color:'var(--cyan-500)',   bg:'rgba(8,145,178,0.1)'   },
        ...
        <div className="grid-3" style={{ marginBottom:'24px' }}>
          {[
            { to:'/predict',      emoji:'🔬', title:'Run AI Prediction',  desc:"Heart, Liver or Parkinson's — instant ML analysis",  color:'var(--cyan-500)',   bg:'rgba(8,145,178,0.1)',   bdr:'rgba(8,145,178,0.2)'   },
            { to:'/doctors',      emoji:'👨‍⚕️',title:'Find Specialists',   desc:'Browse doctors, book consultations, pay online',      color:'var(--accent)',     bg:'var(--accent-soft)',    bdr:'var(--accent-glow)'    },
            { to:'/reports',      emoji:'📄', title:'My Health Reports',   desc:'Download AI-generated PDF reports anytime',           color:'var(--violet-500)',bg:'rgba(124,58,237,0.1)', bdr:'rgba(124,58,237,0.2)' },
          ].map((a,i)=>(
            <Link key={i} to={a.to} style={{ textDecoration:'none' }} className={`animate-fadeInUp stagger-${i+4}`}>
              <div className="card" style={{ display:'flex', alignItems:'center', gap:'16px', border:`1.5px solid ${a.bdr}`, background:`linear-gradient(145deg, var(--bg-surface) 0%, ${a.bg} 100%)`, cursor:'pointer', transition:'all 0.25s' }}
                ...
                {appointments.filter(a=>a.status!=='Cancelled').slice(0,5).map(appt=>{
                  ...
                  return (
                    <div key={appt._id} style={{ display:'flex', alignItems:'center', gap:'12px', padding:'13px', background:'var(--bg-raised)', borderRadius:'14px', border:'1.5px solid var(--border-subtle)' }}>
                      <div style={{ width:'42px', height:'42px', borderRadius:'12px', background:'rgba(8,145,178,0.1)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'21px', border:'1.5px solid rgba(8,145,178,0.2)', flexShrink:0 }}>👨‍⚕️</div>
                      <div style={{ flex:1 }}>
        ...
        {/* Health Tip */}
        <div className="animate-fadeInUp" style={{ marginTop:'24px', padding:'20px 24px', borderRadius:'18px', background:'var(--accent-soft)', border:'1.5px solid var(--accent-glow)', display:'flex', alignItems:'flex-start', gap:'16px' }}>
