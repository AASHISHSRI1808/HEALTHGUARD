...
export default function Navbar() {
  ...
  const roleConfig = {
    ...
    doctor:  { color:'var(--cyan-500)',   bg:'rgba(8,145,178,0.12)',   label:'Doctor'  },
      ...
      {user ? (
        <>
          <ul className="nav-links">
            ...
            {user.role === 'patient' && (<>
              <li><Link to="/predict"      className={isActive('/predict') ?'active':''}>🔬 Predict</Link></li>
            ...
            {user.role === 'doctor' && (
              <li><Link to="/doctor-predict" className={isActive('/doctor-predict')?'active':''}>🔬 Run Prediction</Link></li>
            )}
          </ul>
